...
if os_item == "Windows":
    ...
    if str(os_name()) == "kali":
        ...
        if "Basic Setup" in x_item:
            ...
            items = []
            questions = [inquirer.Checkbox('scripts',message="use space to select app(s) and press enter for install ",choices=['torghost','ghidra','alfa-driver','pattern-generator','autorecon','bloodhound','burpsuite','crackmapexec','curl-http3','dotdotpwn','gitdumper','gobuster','impacket','jd-gui','john','maltego','metasploit','mysql','nvidia','padbuster','powershell','ridenum','rsacrack','searchsploit','seclist','setoolkit','shodan','winpayloads','ysoserial','xsstrike','microsoft-team','tor','brave'],)]
            try:
                answers = inquirer.prompt(questions)
                if len(answers['scripts']) == 0:
                    print(Fore.RED+"[-] No script selected !")
